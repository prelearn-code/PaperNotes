...
- **数学原理：** 构造一个伪随机序列 $x_{i+1} = f(x_i) \pmod n$（通常 $f(x) = x^2 + c$）。如果存在因子 $p$，那么序列在模 $p$ 下会比在模 $n$ 下更快出现循环（即“Rho”字形的路径）。
    
- **过程：** 1. 使用 Floyd 判圈算法（快慢指针）寻找 $x_i \equiv x_j \pmod p$。
    
    2. 计算 $gcd(|x_i - x_j|, n)$。如果结果在 $1$ 和 $n$ 之间，则找到了因子。
...
- **过程：** 1. 选取一个基数 $a$（通常为 2）。
    ...
    2. 计算 $M = \prod q_i^{e_i}$（所有小于 $B$ 的质幂之积）。
